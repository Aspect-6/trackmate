...
const VerifyAndChangeEmailAction: React.FC<ActionHandler.VerifyAndChangeEmailActionProps> = ({ oobCode }) => {
    ...
    useEffect(() => {
        ...
        const handleEmailChange = async () => {
            try {
                ...
                if (import.meta.env.DEV) console.error('Error verifying email change:', error)
                ...
                switch (error.code) {
                    case 'auth/expired-action-code':
                        setErrorMessage('This email change link has expired. Please request a new one.')
                        break
                    case 'auth/invalid-action-code':
                        setErrorMessage('This email change link is invalid or has already been used.')
                        break
                    case 'auth/user-disabled':
                        setErrorMessage('This account has been disabled.')
                        break
                    case 'auth/user-not-found':
                        setErrorMessage('No account found for this email change link.')
                        break
                    default:
                        setErrorMessage('Failed to verify email change. Please request a new link.')
                }
                setState('error')
            }
        }
    ...
    return (
        <div className="min-h-dvh flex items-center justify-center p-4">
            ...
            >
                <HomeLink />
                <Title>
                    {state === 'loading' && 'Verifying email change'}
                ...
                <div className="flex flex-col items-center space-y-6">
                    ...
                    <p className="text-sm text-center" style={{ color: AUTH.TEXT_SECONDARY }}>
                        {state === 'loading' && 'Please wait while we verify your email change...'}
                        {state === 'success' && 'Your email has been successfully changed. Please sign in with your new email.'}
...
export default VerifyAndChangeEmailAction
