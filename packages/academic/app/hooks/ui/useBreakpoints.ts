...
const LANDSCAPE_PHONE_BREAKPOINT = "(max-height: 600px) and (orientation: landscape)"
 ...
 * - `isLandscapePhone`: max-height 600px in landscape
 */
export const useBreakpoints = () => {
    ...
    const [isLandscapePhone, setIsLandscapePhone] = useState(() => window.matchMedia(LANDSCAPE_PHONE_BREAKPOINT).matches)

    useEffect(() => {
        const mobileQuery = window.matchMedia(MOBILE_BREAKPOINT)
        const tabletQuery = window.matchMedia(TABLET_BREAKPOINT)
        const desktopQuery = window.matchMedia(DESKTOP_BREAKPOINT)
        const landscapePhoneQuery = window.matchMedia(LANDSCAPE_PHONE_BREAKPOINT)
        ...
        const handleLandscapePhone = (e: MediaQueryListEvent) => setIsLandscapePhone(e.matches)

        mobileQuery.addEventListener("change", handleMobile)
        tabletQuery.addEventListener("change", handleTablet)
        desktopQuery.addEventListener("change", handleDesktop)
        landscapePhoneQuery.addEventListener("change", handleLandscapePhone)
        ...
        return () => {
            ...
            landscapePhoneQuery.removeEventListener("change", handleLandscapePhone)
        }
    }, [])

    return { isMobile, isTablet, isDesktop, isLandscapePhone }
}
